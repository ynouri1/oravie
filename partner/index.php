...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username && $password) {
        try {
            $pdo = getDB();
            ensureSchema($pdo);
            $stmt = $pdo->prepare("SELECT id, password, role, nom, actif FROM admins WHERE username = :u LIMIT 1");
            $stmt->execute([':u' => $username]);
            $user = $stmt->fetch();

            if ($user && password_verify($password, $user['password'])) {
                if (($user['role'] ?? '') !== 'commercial') {
                    $error = 'Ce compte n\'est pas un compte commercial.';
                    ...
                    $_SESSION['partner_user'] = $username;
                    $_SESSION['partner_nom']  = $user['nom'] ?: $username;
...
<head>
  ...
  <title>ORAVIE · Espace Commercial</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { min-height:100vh; background:linear-gradient(135deg,#2F4B3C,#4A735C); display:flex; align-items:center; justify-content:center; font-family:'Segoe UI',sans-serif; padding:1rem; }
